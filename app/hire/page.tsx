...
export default async function HirePage() {
  ...
  let seekerCount = 0
  try {
    seekerCount = await withRetry(() => db.seekerProfile.count())
  } catch (error) {
  ...
  return (
    <div>
      <section className="border-b border-brand-border bg-brand-purple/[0.04]">
        <div className="max-w-4xl mx-auto px-5 py-16 md:py-20">
          <span className="inline-block text-xs font-semibold uppercase tracking-wider text-brand-purple mb-4">
            For businesses
          </span>
          <h1 className="text-4xl md:text-5xl font-bold tracking-tight leading-[1.08] mb-5">
            Hire remote talent without paying<br className="hidden md:block" /> for the privilege
          </h1>
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {PROMISES.map(p => (
            <div key={p.label} className="card p-6">
              <p className="text-3xl font-bold text-brand-purple mb-1">{p.figure}</p>
      ...
      <section className="max-w-4xl mx-auto px-5 pb-14">
        <h2 className="text-2xl font-bold tracking-tight mb-6">How hiring works here</h2>
        <div className="card divide-y divide-brand-border">
          {STEPS.map(s => (
            <div key={s.n} className="p-6 flex gap-5 items-start flex-wrap">
              <span className="text-sm font-mono text-brand-muted pt-1">{s.n}</span>
      ...
      <section className="max-w-4xl mx-auto px-5 py-16 text-center">
        ...
        <div className="flex flex-wrap gap-3 justify-center">
          ...
          <Link href="/register?role=employer" className="btn-secondary">Create a free account</Link>
        </div>
      </section>
    </div>
  )
